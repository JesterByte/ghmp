...
class BurialPricingModel extends Model {
    public function getPricingData() {
        $stmt = $this->db->prepare("SELECT * FROM burial_pricing");
        $stmt->execute();
        return $stmt->fetchAll();
    }

    public function getLotPricingData() {
    ...
    public function getEstatePricingData() {
        $stmt = $this->db->prepare("SELECT * FROM burial_pricing WHERE category = :category");
        $category = "Estate";
        $stmt->bindParam(":category", $category, PDO::PARAM_STR);
        $stmt->execute();
    ...
    public function getPrice($category, $burialType) {
        $stmt = $this->db->prepare("SELECT price FROM burial_pricing WHERE category = :category AND burial_type = :burial_type");
        $stmt->bindParam(":category", $category, PDO::PARAM_STR);
        $stmt->bindParam(":burial_type", $burialType, PDO::PARAM_STR);
        $stmt->execute();
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }

    public function updatePrice($category, $burialType, $newPrice) {
        $stmt = $this->db->prepare("UPDATE burial_pricing SET price = :price WHERE category = :category AND burial_type = :burial_type");
        $stmt->bindParam(":category", $category, PDO::PARAM_STR);
        $stmt->bindParam(":burial_type", $burialType, PDO::PARAM_STR);
    ...
    //     $stmt = $this->db->prepare("SELECT vat, memorial_care_fee, cash_sale_discount, six_months_discount, down_payment_rate, one_year_interest_rate, two_years_interest_rate, three_years_interest_rate, four_years_interest_rate, five_years_interest_rate FROM phase_pricing LIMIT 1");
    //     $stmt->execute();
    //     return $stmt->fetch(PDO::FETCH_ASSOC);
    // }
    ...
    //     $stmt = $this->db->prepare("UPDATE phase_pricing 
    //     SET 
    ...
    //         balance = :new_balance,
    //         {$amortizationPlaceholders}
    //     WHERE phase = :phase AND lot_type = :lot_type
    //     ");
    ...
    //     $stmt->bindParam(":lot_type", $lotType, PDO::PARAM_STR);
