...
class LotReservationRequestsController extends BaseController
{
    ...
    public function submitLotReservationConfirmation()
    {
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            switch ($status) {
                case "Confirmed":
                    $lotReservationRequestsModel->approveLotReservation($lotId, $reserveeId);
                    $message = "The lot reservation has been approved successfully!";
                    $notificationMessage = "Your lot reservation for Lot #$lotId has been approved.";
                    $emailSubject = "Lot Reservation Approved";
                    $emailBody = '
                        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
                            ...
                            <p>Your reservation for <strong>Lot #' . htmlspecialchars($lotId) . '</strong> has been <strong style="color: #28a745;">Approved</strong>.</p>
                ...
                case "Cancelled":
                    $lotReservationRequestsModel->cancelLotReservation($lotId, $reserveeId, "Admin: $cancelReason");
                    $lotReservationRequestsModel->setLotStatus($lotId, "Available");
                    $message = "The lot reservation has been cancelled successfully!";
                    $notificationMessage = "Your lot reservation for Lot #$lotId has been cancelled.";
                    $emailSubject = "Lot Reservation Cancelled";
                    $emailBody = '
                        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
                            <h2 style="color: #333; text-align: center;">Lot Reservation Cancelled</h2>
                            <p>Dear <strong>' . htmlspecialchars($customerName) . '</strong>,</p>
                            <p>We regret to inform you that your reservation for <strong>Lot #' . htmlspecialchars($lotId) . '</strong> has been <strong style="color: #dc3545;">Cancelled</strong>.</p>
            ...
            $this->redirect(BASE_URL . "/lot-reservation-requests", DisplayHelper::$checkIcon, $message, "Operation Successful");
        }
    }
