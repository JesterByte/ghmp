...
$estateQuery = "
    SELECT er.id AS reservation_id, er.estate_id AS asset_id, er.reservee_id, c.email_address, 'Estate Reservation' AS reservation_type,
        ...
        LEFT JOIN estate_six_months_due_dates AS smdd ON sm.id = smdd.six_months_id AND DATEDIFF(CURDATE(), smdd.due_date) > 0
    ...
    foreach ($groupedResults as $emailAddr => $reservations) {
        ...
        foreach ($reservations as $reservation) {
            if (!empty($reservation['cash_sale_due_date'])) {
                $message .= "<tr><td>{$reservation['reservation_type']}</td><td>{$reservation['asset_id']}</td><td>Cash Sale</td><td>{$reservation['cash_sale_due_date']}</td></tr>";
            }
            if (!empty($reservation['six_month_due_date'])) {
                $message .= "<tr><td>{$reservation['reservation_type']}</td><td>{$reservation['asset_id']}</td><td>Six Months</td><td>{$reservation['six_month_due_date']}</td></tr>";
            }
            if (!empty($reservation['installment_dp_due_date'])) {
                $message .= "<tr><td>{$reservation['reservation_type']}</td><td>{$reservation['asset_id']}</td><td>Installment DP</td><td>{$reservation['installment_dp_due_date']}</td></tr>";
            }
            if (!empty($reservation['installment_next_due_date'])) {
                $message .= "<tr><td>{$reservation['reservation_type']}</td><td>{$reservation['asset_id']}</td><td>Installment Next</td><td>{$reservation['installment_next_due_date']}</td></tr>";
            }
        }
        ...
        $message .= "
            ...
            <div style='background-color: #f4f4f4; text-align: center; padding: 10px; font-size: 12px;'>
                &copy; " . date('Y') . " Green Haven Memorial Park.
            </div>
        </div>";
